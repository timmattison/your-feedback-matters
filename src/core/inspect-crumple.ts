...
// ~75% flat (t = INSPECT_CRUMPLE_T ≈ 0.25) — `openingT` maps inspect progress
...
// Un-crumple: full wad (t = 1) → ~75% flat (t = INSPECT_CRUMPLE_T).
export function openingT(p: number): number {
  return lerp(1, INSPECT_CRUMPLE_T, easeInOutCubic(p));
}

// Re-crumple: ~75% flat (t = INSPECT_CRUMPLE_T) → full wad (t = 1). Mirror of opening.
